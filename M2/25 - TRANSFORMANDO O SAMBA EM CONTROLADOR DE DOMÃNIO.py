TRANSFORMANDO O SAMBA EM CONTROLADOR DE DOMÃNIO

Criando uma cÃ³pia do "/etc/samba/smb.conf" Standard

# cd /etc/samba
# cp smb.conf std_smb.conf

Modificando o /etc/samba/smb.conf

### Novo smb.conf modificado para Controlador de DomÃ­nio ###
[global]
...
[Marketing]
        ...
        ## Lixeira do compartilhamento[netlogon]
        ;;; NÃ£o esquecer de criat o dirtÃ³rio /var/samba/netlogon
        comment         = Scripts de logon
        path            = /var/samba/netlogon
        guest ok        = yes
...
[Diretoria]
        ...
        recycle:exclude         = *.bak, *.iso, *.obj
        recycle:exclude_dir     = tmp, cache
### Novo smb.conf modificado para Controlador de DomÃ­nio - FIM ###

Criando os diretÃ³rio para PROFILE e NETLOGON:

# mkdir -p /var/samba/{netlogon,profiles}

O diretÃ³rio "/var/samba/profiles" tem como finalidade armazenar o Perfil MÃ³vel dos usuÃ¡rio.

O diretÃ³rio "/var/samba/profiles" serve para armazenar os Scripts de Logon do domÃ­nio.

Ajustando as permissÃµes do /var/samba/profile:

# chmod 1777 /var/samba/profiles
Adicionando o grupo computadores para funcionar como grupo base das mÃ¡quinas do domÃ­nios:

# addgroup computadores
